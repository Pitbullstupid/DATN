...
export const createCourse = async (req, res) => {
  try {
    ...
    if (!schedules?.length)
      return res
        .status(400)
        .json({ status: "error", message: "Vui lòng cung cấp thời khóa biểu" });
    if (!startDate || !endDate || !totalSessions)
      return res
        .status(400)
        .json({ status: "error", message: "Thiếu thông tin khóa học" });
    ...
    const totalPrice = pricePerSession
      ? parseFloat(pricePerSession) * parseInt(totalSessions)
      : null;
    ...
    // Notify student cần thanh toán
    await notifyPaymentRequired(course, booking.studentId);
  ...
  try {
    ...
    if (!isTutor)
      return res.status(403).json({
        status: "error",
        message: "Chỉ gia sư mới có thể bắt đầu lớp học",
      });
    if (course.status !== "UPCOMING")
      return res.status(400).json({
        status: "error",
        message: `Lớp đang ở trạng thái "${course.status}"`,
      });
    ...
    // Notify student lớp đã bắt đầu
    await notifyCourseStarted(course, course.studentId);
...
export const requestEndCourse = async (req, res) => {
  try {
    ...
    if (course.status !== "ONGOING")
      return res.status(400).json({
        status: "error",
        message: "Chỉ có thể kết thúc lớp đang ONGOING",
      });
    ...
    if (updated.tutorConfirmedEnd && updated.studentConfirmedEnd) {
      ...
      // Giải phóng tiền escrow
      await releasePayment(id);

      // Notify cả 2
      const tutorUserId = course.tutorProfile.userId;
      await notifyCourseCompleted(course, course.studentId, tutorUserId);

      return res.status(200).json({
        status: "success",
        message:
          "Cả hai đã xác nhận — khóa học đã kết thúc! Học phí đã được giải phóng.",
        data: { completed: true },
      });
    }

    // Chưa đủ 2 bên → notify bên còn lại
    const tutorUserId = course.tutorProfile.userId;
    if (isTutor) {
      // Tutor vừa confirm → notify student
      await notifyEndCourseRequested(course, course.studentId);
    } else {
      // Student vừa confirm → notify tutor đang chờ xử lý
      await notifyEndCourseWaiting(course, tutorUserId);
    }
...
export const cancelCourse = async (req, res) => {
  try {
    ...
    if (!["PENDING_PAYMENT", "UPCOMING", "ONGOING"].includes(course.status))
      return res.status(400).json({
        status: "error",
        message: `Không thể huỷ lớp ở trạng thái "${course.status}"`,
      });
    ...
    // Xử lý thanh toán dựa trên ai huỷ lớp
    ...
    if (payment?.status === "PAID") {
      if (isTutor) {
        // Gia sư cancel → hoàn tiền cho học sinh
        ...
        // Học sinh cancel → Cộng tiền vào ví gia sư (không hoàn cho học sinh)
    ...
    const messageDetail =
      !payment || payment.status !== "PAID"
        ? ""
        : isTutor
          ? " và hoàn tiền cho học viên"
          : " (học phí được cộng cho gia sư)";

    res.status(200).json({
      status: "success",
      message: "Đã huỷ lớp học" + messageDetail,
    });
  } catch (err) {
    res
      .status(err.status || 500)
...
export const confirmSession = async (req, res) => {
  try {
    ...
    if (!session)
      return res
        .status(404)
        .json({ status: "error", message: "Không tìm thấy buổi học" });

    if (session.tutorConfirmed && session.studentConfirmed)
      return res.status(400).json({
        status: "error",
        message: "Buổi học đã được cả 2 bên xác nhận",
      });

    if (isTutor && session.tutorConfirmed)
      return res
        .status(400)
        .json({ status: "error", message: "Bạn đã xác nhận buổi học này rồi" });
    ...
    if (courseCheck.status !== "ONGOING")
      return res.status(400).json({
        status: "error",
        message: "Khóa học chưa bắt đầu hoặc đã kết thúc",
      });

    if (!["ONGOING", "COMPLETED"].includes(session.status))
      return res.status(400).json({
        status: "error",
        message: "Buổi học chưa diễn ra, không thể xác nhận",
      });
    ...
    if (updated.tutorConfirmed && updated.studentConfirmed) {
      ...
      // Lấy thông tin course để notify
      const courseForNotif = await prisma.courseClass.findUnique({
        where: { id },
        select: {
          subject: true,
          studentId: true,
      ...
      await notifySessionConfirmed(
        { ...updated, courseClassId: id },
        courseForNotif.subject,
        courseForNotif.studentId,
        courseForNotif.tutorProfile.userId,
      );
    ...
    // Chưa đủ 2 bên → notify bên còn lại chờ xác nhận
    const courseForNotif = await prisma.courseClass.findUnique({
      where: { id },
      select: {
        subject: true,
    ...
    const waitingUserId = isTutor
      ? courseForNotif.studentId
      : courseForNotif.tutorProfile.userId;
    await notifySessionConfirmWait(
      { ...updated, courseClassId: id, sessionNumber: session.sessionNumber },
      courseForNotif.subject,
      waitingUserId,
    ...
    res.status(200).json({
      ...
      data: {
        bothConfirmed: false,
        tutorConfirmed: updated.tutorConfirmed,
        studentConfirmed: updated.studentConfirmed,
      },
    });
  } catch (err) {
    res
...
export const updateSession = async (req, res) => {
  try {
    ...
    if (!isTutor)
      return res.status(403).json({
        status: "error",
        message: "Chỉ gia sư mới có thể cập nhật buổi học",
      });
    ...
    if (session.tutorConfirmed && session.studentConfirmed)
      return res.status(400).json({
        status: "error",
        message: "Buổi học đã được cả 2 bên xác nhận, không thể thay đổi",
      });
    ...
    if (!valid.includes(status))
      return res
        .status(400)
        .json({ status: "error", message: "Trạng thái không hợp lệ" });
...
export const reviewCourse = async (req, res) => {
  try {
    ...
    if (!isStudent)
      return res
        .status(403)
        .json({ status: "error", message: "Chỉ học sinh mới có thể đánh giá" });
    if (course.status !== "COMPLETED")
      return res.status(400).json({
        status: "error",
        message: "Chỉ đánh giá được khóa học đã hoàn thành",
      });
    if (course.review)
      return res
        .status(400)
        .json({ status: "error", message: "Bạn đã đánh giá khóa học này rồi" });
    if (!rating || rating < 1 || rating > 5)
      return res
        .status(400)
        .json({ status: "error", message: "Rating phải từ 1 đến 5" });
...
export const getMessages = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: "success", data: { messages: messages.reverse() } });
  } catch (err) {
    res
      .status(err.status || 500)
      .json({ status: "error", message: err.message });
...
export const sendMessage = async (req, res) => {
  try {
    ...
    if (!content?.trim())
      return res
        .status(400)
        .json({ status: "error", message: "Nội dung không được để trống" });
    await getCourseWithAccess(id, req.user.id);
